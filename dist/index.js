...
unwrapExports(async);
var async_1 = async.waterfall;
var async_2 = async.parallel;
...
function startStepFunctions(files, cb6) {
    var params = {
        stateMachineArn: 'arn:aws:states:us-east-1:254372949584:stateMachine:TranscriptBuilder',
        input: JSON.stringify({ files: files })
    };
    const stepfunctions = new awsSdk.StepFunctions({ region: 'us-east-1' });
    stepfunctions.startExecution(params, cb6);
}
exports.startTranscribe = (event, context) => {
...
exports.checkTranscribeJobs = (event) => {
    const isSuc = (s) => (s.TranscriptionJob.TranscriptionJobStatus === 'COMPLETED');
    return Promise.all(event.Jobs.map(checkTranscribeJob)).then(d => {
...
exports.buildTranscript = (event) => {
    ...
    return Promise.all(chunks).then(c => c.reduce((acc, val) => {
...
function flattenTranscript(trans) {
...
function startTranscribeJob(s3File) {
    var params = {
        LanguageCode: 'en-US',
        Media: {
            MediaFileUri: s3File,
    ...
    var transcribeservice = new awsSdk.TranscribeService({ region: 'us-east-1' });
...
function checkTranscribeJob(transcriptionJob) {
    var transcribeservice = new awsSdk.TranscribeService({ region: 'us-east-1' });
    return new Promise((resolve, reject) => {
...
function splitMp3(filepath, cb2) {
...
function listOutputFiles(dir, cb3) {
...
function uploadOutputFiles(files, cb4) {
    async_2(files.map(function (file) {
        return (c) => uploadFile(file, c);
    }), cb4);
}
function uploadFile(file, cb5) {
